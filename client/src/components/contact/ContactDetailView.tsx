...
export function ContactDetailView({
  ...
  // Meeting drawer state
  const quickSlots = useMemo(() => getQuickTimeSlots() as QuickSlot[], []);
  const [showMeeting, setShowMeeting] = useState(false);

  const slotToDate = (s?: QuickSlot): Date => {
    try {
      const t = s?.getTime?.();
  ...
  const handleGenerateFollowUp = async () => {
    if (!followUpGoal.trim()) {
      toast({
        title: "Goal required",
        description:
          "Tell Carda what you want to achieve (e.g., book a call, send pricing, intro).",
        variant: "destructive",
      });
      return;
    }
  ...
  const quickActions: QuickAction[] = useMemo(() => {
    ...
    if (hubspotStatus?.connected) {
      const isSynced = contactV2?.timeline?.some(
        (t) => t.type === "hubspot_synced"
      );
      actions.push({
        id: "hubspot",
  ...
  return (
    <div className="flex flex-col min-h-full pb-24" data-testid="contact-detail-view">
      ...
      {/* Meeting Drawer */}
      <Drawer open={showMeeting} handleOnly onOpenChange={setShowMeeting}>
        <DrawerContent className="bg-background/60 backdrop-blur-2xl border border-white/10 rounded-t-2xl h-[85vh] max-h-[85vh] overflow-hidden flex flex-col">
          <DrawerHeader>
            <DrawerTitle>Meeting Invite</DrawerTitle>
          </DrawerHeader>
          ...
          <div className="flex-1 overflow-y-auto overscroll-contain px-4 pb-4 space-y-4">
            <div className="space-y-2">
              ...
              <div className="flex gap-2 overflow-x-auto pb-1">
                {quickSlots.map((s) => {
                  const d = slotToDate(s);
                  return (
                    <Button
                      key={s.label}
                      variant={
                        toDatetimeLocalValue(meetingStart) === toDatetimeLocalValue(d)
                          ? "default"
                          : "outline"
                      }
                      size="sm"
                      onClick={() => setMeetingStart(d)}
                      className="shrink-0"
                    >
                      {s.label}
                    </Button>
