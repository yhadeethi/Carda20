...
export function HubSpotProfile({ open, onOpenChange }: HubSpotProfileProps) {
  ...
  const { data: hubspotStatus, isLoading: statusLoading } = useQuery<{
    connected: boolean;
  ...
  const { data: events = [] } = useQuery<UserEvent[]>({
    ...
    enabled: open && !!hubspotStatus?.connected,
  });
  ...
  const exportTimelineMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await apiRequest("POST", "/api/hubspot/export-timeline", {
            ...
            <Card>
              <CardContent className="p-4 space-y-3">
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 rounded-full bg-[#FF7A59]/10 flex items-center justify-center">
                    <SiHubspot className="w-5 h-5 text-[#FF7A59]" />
                  </div>
                  <div className="flex-1">
                    <p className="text-sm font-medium">Connect your HubSpot account</p>
              ...
              {exportView === "main" && (
                <>
                  ...
                  <div className="space-y-1.5">
                    <h3 className="text-sm font-semibold tracking-tight px-1">Export to HubSpot</h3>
                    ...
                    <Card className="hover-elevate">
                      <CardContent className="p-0">
                        <button
                          ...
                          data-testid="button-hubspot-export-contacts"
                        >
                          <div className="flex items-center gap-3 min-w-0">
                          ...
                          <CardContent className="p-0 divide-y divide-border/50">
                            {contactsForTimeline.slice(0, 15).map((contact: any) => (
                              <button
                                ...
                                data-testid={`button-hubspot-timeline-${contact.id}`}
                              >
                                <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="space-y-3">
                  ...
                  {timelineEvents.length > 0 && (
                    <div className="flex items-center justify-between px-1">
                      <button
                        ...
                        data-testid="button-timeline-select-all"
                      >
                      ...
                      {timelineEvents.length === 0 ? (
                        ...
                        timelineEvents.map((entry) => (
                          <button
                            ...
                            data-testid={`checkbox-timeline-${entry.id}`}
                          >
                  ...
                  {selectedTimelineIds.length > 0 && (
                    <Button
                      ...
                      data-testid="button-hubspot-export-timeline"
                    >
