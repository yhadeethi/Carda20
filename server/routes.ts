...
Return ONLY valid JSON, no markdown or explanation.`;
  ...
  // ============================================
  // User Events API Endpoints
  // ============================================

  // User Events - GET all events for user
  ...
  // User Events - CREATE a new event
  app.post("/api/user-events", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const userId = await getCurrentUserId(req);
      const { title, startAt, endAt, locationLabel, lat, lng, tags, notes, source } = req.body;
  ...
  // User Events - GET a specific event
  app.get("/api/user-events/:id", isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      const event = await storage.getUserEvent(userId, eventId);
  ...
  app.patch("/api/user-events/:id", isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      // Verify ownership
      const existing = await storage.getUserEvent(userId, eventId);
      if (!existing) {
        return res.status(404).json({ error: "Event not found" });
      }

      const { title, startAt, endAt, locationLabel, lat, lng, tags, notes } = req.body;
  ...
  // User Events - END an event (set endAt to now)
  app.post("/api/user-events/:id/end", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const userId = await getCurrentUserId(req);
      const eventId = parseInt(req.params.id);
      ...
      const event = await storage.updateUserEvent(eventId, { endAt: new Date() });
      res.json(event);
    } catch (error) {
      console.error("[UserEvents] Error ending event:", error);
      res.status(500).json({ error: "Failed to end event" });
    }
  });

  // User Events - DELETE an event
  ...
  // User Events - GET or CREATE draft event
  app.post("/api/user-events/draft", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const userId = await getCurrentUserId(req);
      const event = await storage.getOrCreateDraftEvent(userId);
      res.json(event);
    } catch (error) {
      console.error("[UserEvents] Error getting/creating draft:", error);
      res.status(500).json({ error: "Failed to get or create draft event" });
    }
  });

  // User Events - FINALIZE a draft event
  app.post("/api/user-events/:id/finalize", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const userId = await getCurrentUserId(req);
      const eventId = parseInt(req.params.id);
      ...
      const { title, notes, tags, locationLabel } = req.body;

      if (!title) {
        return res.status(400).json({ error: "Title is required" });
      }

      const event = await storage.finalizeDraftEvent(userId, eventId, {
        title,
        notes,
        tags,
        locationLabel,
      ...
      console.error("[UserEvents] Error finalizing event:", error);
      res.status(500).json({ error: "Failed to finalize event" });
    }
  });

  // User Event Contacts - GET contacts for an event
  app.get("/api/user-events/:id/contacts", isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      const contacts = await storage.getUserEventContacts(userId, eventId);
      res.json(contacts);
    } catch (error) {
      console.error("[UserEventContacts] Error fetching contacts:", error);
      res.status(500).json({ error: "Failed to fetch event contacts" });
    }
  });

  // User Event Contacts - ATTACH contacts to an event
  app.post("/api/user-events/:id/attach-contacts", isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      const contactRefs = contacts.map((c: any) => ({
        contactIdV1: c.contactIdV1 || c.v1 || c.id,
        contactIdV2: c.contactIdV2 || c.v2 || null,
      }));

      const attached = await storage.attachContactsToEvent(userId, eventId, contactRefs);
      res.json(attached);
    } catch (error) {
      console.error("[UserEventContacts] Error attaching contacts:", error);
  ...
  app.get("/api/run-migrations", isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      results.push("🚀 Running Migration 0003: User Events...");
      await db.execute(migration0003);
      results.push("✅ Migration 0003 completed successfully!");
